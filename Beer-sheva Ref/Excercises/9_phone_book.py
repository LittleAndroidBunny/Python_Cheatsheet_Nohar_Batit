...
def add_person(book: list, person: List[Union[str, list]]) -> list:
    """
    add a person to the phone book
    :param List book: phone book
    :param List[str, list] person: list containg person to add
    :return: the phone book list received, appended with the new person
    :rtype: List
    """
    book.append(person)
    if type(book) != list:
...
def remove_person_by_name(book: list, person_name: str) -> list:
    """
       removes a person from phone book if found
       :param List book: phone book
       :param str person_name:
       :return: phone book without person
       :rtype: list
       """
    # bad code! - what happens if we look to for a missing number? error!
    # return book.remove([x for x in book if x[0] == person_name][0])
...
test = remove_person_by_name(phone_book, 'Donald')
